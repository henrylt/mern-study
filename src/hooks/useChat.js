...
export function useChat() {
  ...
  const [currentRoom, setCurrentRoom] = useState('public') // for joining and switching rooom
  function switchRoom(room) {
    setCurrentRoom(room)
  }
  function joinRoom(room) {
    // room join and switch
  ...
  async function sendMessage(message) {
    if (message.startsWith('/')) {
      ...
      const [command, ...args] = message.substring(1).split(' ') // manage chat room
      switch (command) {
        case 'join': {
          if (args.length === 0) {
      ...
      socket.emit('chat.message', currentRoom, message) // room manage
    }
